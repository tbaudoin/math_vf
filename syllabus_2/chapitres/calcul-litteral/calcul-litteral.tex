...
\begin{exercise}
  ...
  \vspace*{1cm}
  nb: une différence revient à additioner à un nombre, un nombre négatif.
  \vspace*{2cm}
  \begin{center}
...
\adf[Tableau de synthèse]
\begin{tabular}{\linewidth}{|X|X|X|}
  \hline
    \textbf{S'il y a une "+" devant la paranthèse} 
    & textbf{$a + (b-c) = a + b - c$} 
    & \textbf{On enlève la paranthèse et rien ne change}\\
  \hline
    \textbf{S'il y a un "-" devant la paranthèse} 
    & \textbf{$a - (b-c) = a - b + c$} 
    & \adf[On distribue le "-" à chaque terme de la paranthèse]\\
  \hline
    \textbf{S'il y a un "." devant la paranthèse} 
    & \textbf{$a.(b+c) = ab + ac$} 
    & \textbf{On effectue une distributivité simple}\\
  \hline
\end{tabular}
